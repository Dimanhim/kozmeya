...
class AdminUsers extends ActiveRecord implements IdentityInterface
{
    ...
    public function updateRelations(){
        ...
        \Yii::$app->db->createCommand()->delete('permissions', ['user_id' => $this->id,])->execute();

        if(isset($data["permissions"])){
            foreach($data["permissions"] as $access=>$sections){
                foreach($sections as $section){
                    $permissionmodel = new Permissions();
                    $permissionmodel->user_id = $this->id;
                    $permissionmodel->section = $section;
                    $permissionmodel->access = $access;
